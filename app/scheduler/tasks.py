...
def update_stock_data():
    """
    Step 1: Update Indian stock data from yfinance
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"⚠️ Data update failed with code {result.returncode}")
            print(result.stderr)
            return False
    
    except subprocess.TimeoutExpired:
        print("❌ Data update timed out (>10 minutes)")
        return False
...
def train_model():
    """
    Step 2: Train FinGAT model with updated data
    Runs: scripts/train_model.py
    """
    print("=" * 60)
    print(f"🏋️ Starting model training - {datetime.now()}")
    print("=" * 60)
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"⚠️ Model training failed with code {result.returncode}")
            print(result.stderr)
            return False
    
    except subprocess.TimeoutExpired:
        print("❌ Model training timed out (>1 hour)")
        return False
    except Exception as e:
...
def daily_pipeline():
    ...
    1. Update stock data from yfinance
    2. Retrain FinGAT model
    ...
    if not data_success:
        print("\n❌ Pipeline failed at data update stage")
        return
    ...
    if not reload_success:
        print("\n⚠️ Model trained but reload failed")
    ...
    print(f"   Data Update: {'✅' if data_success else '❌'}")
    print(f"   Model Training: {'✅' if train_success else '❌'}")
